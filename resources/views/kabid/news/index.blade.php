                ...
                <table id="tableMenu" class="table table-sm table-striped table-row-bordered border rounded">
                    <thead>
                        <tr class="fw-semibold fs-6 text-muted">
                            ...
                            <th class="border border-1 text-center">Penulis</th>
                       ...
                       @foreach ($news as $item)
                            <tr>
                                ...
                                <td class="border border-1 text-center">
                                    <a href="{{ route('k-news.edit', $item->id) }}" class="btn btn-icon btn-primary w-35px h-35px mb-sm-0 mb-3">
                                    ...
                                    <div class="modal fade text-start" tabindex="-1" id="confirmDelete{{ $item->id }}">
                                        <form action="{{ route('k-news.destroy', $item->id) }}" method="POST">
                                            @csrf
                                            @method('delete')
                                ...
                                <td class="border border-1">{{ $item->_user->username ?? '-'}}</td>
                                <td class="border border-1 text-center">
                    ...
                    <tbody>
                       @foreach ($news as $item)
                            <tr>
                                <td class="text-center border border-1">{{ $loop->iteration }}</td>
                                <td class="border border-1 text-center">
                                    <a href="{{ route('op-news.edit', $item->id) }}" class="btn btn-icon btn-primary w-35px h-35px mb-sm-0 mb-3">
                                        <div class="d-flex justify-content-center">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </div>
                                    </a>
                                    ...
                                    <div class="modal fade text-start" tabindex="-1" id="confirmDelete{{ $item->id }}">
                                        <form action="{{ route('op-news.destroy', $item->id) }}" method="POST">
                                            @csrf
                                            @method('delete')
                                            <div class="modal-dialog">
                                                <div class="modal-content">
...
<div class="container-fluid datatable-init">
    <div class="card">
        <div class="card-header pb-0 card-no-border">
            <h5>Daftar Berita Bappeda Bidang {{$bidang->label}}</h5>
        </div>
        <div class="card-body">
                ...
                <table class="display border table-striped" id="basic-1" style="width: 100%;">
                    <thead>
                        <tr>
                            ...
                            <th class="text-start"> <span class="c-o-light f-w-600">Nama Pembuat</span></th>
                            ...
                            <tr>
                                ...
                                <td class="text-center justify-content-center">
                                    <ul class="action text-center justify-content-center">
                                        <li class="edit"> 
                                            <a href="{{route('k-news.edit', $item->id)}}"><i class="fa-regular fa-pen-to-square"></i>
                                            </a>
                                        </li>
                                        <li class="delete">
